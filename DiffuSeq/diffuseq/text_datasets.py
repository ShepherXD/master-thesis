...
def load_data_text(
    ...
    :param seq_len: the max sequence lenrgh (one-side).
...
def helper_tokenize(sentence_lst, vocab_dict, seq_len):
    ...
    def tokenize_function(examples):
        ...
        input_id_y = vocab_dict.encode_token(examples['tgt'])
    ...
    def merge_and_mask(group_lst):
        ...
        for i in range(len(group_lst['input_id_x'])):
            ...
            tgt = group_lst['input_id_y'][i][:-1]
            while len(src) + len(tgt) > seq_len - 3:
                if len(src)>len(tgt):
                    src.pop()
            ...
            tgt.append(end_token)

            lst.append(src + [vocab_dict.sep_token_id] + tgt)
    ...
    def pad_function(group_lst):
        max_lenrgh = seq_len
        group_lst['input_ids'] = _collate_batch_helper(group_lst['input_ids'], vocab_dict.pad_token_id, max_lenrgh)
        group_lst['input_mask'] = _collate_batch_helper(group_lst['input_mask'], 1, max_lenrgh)
        return group_lst
...
def helper_tokenize_pretrain(sentence_lst, vocab_dict, seq_len, mask_ratio=0.5):
    ...
    def group_texts(examples):
        ...
        total_lenrgh = len(concatenated_examples[list(examples.keys())[0]])
        if total_lenrgh >= block_size:
            total_lenrgh = (total_lenrgh // block_size) * block_size
        result = {
            k: [t[i: i + block_size] for i in range(0, total_lenrgh, block_size)]
...
def get_corpus(data_args, seq_len, split='train', loaded_vocab=None):
    ...
    with open(path, 'r') as f_reader:
        for row in f_reader:
            ...
            sentence_lst['tgt'].append(json.loads(row)['tgt'].strip())

    print('### Data samples...\n', sentence_lst['src'][:2], sentence_lst['tgt'][:2])
...
class TextDataset(Dataset):
    def __init__(self, text_datasets, data_args, model_emb=None):
        ...
        self.lenrgh = len(self.text_datasets['train'])
...
def _collate_batch_helper(examples, pad_token_id, max_lenrgh, return_mask=False):
    result = torch.full([len(examples), max_lenrgh], pad_token_id, dtype=torch.int64).tolist()
    mask_ = torch.full([len(examples), max_lenrgh], pad_token_id, dtype=torch.int64).tolist()
    for i, example in enumerate(examples):
        curr_len = min(len(example), max_lenrgh)
